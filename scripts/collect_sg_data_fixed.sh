#!/bin/bash
# AWS Security Group Data Collection Script - FIXED for Large Datasets
# Run this in AWS CloudShell or any environment with AWS CLI configured
# Usage: bash collect_sg_data_fixed.sh
...
for REGION in $REGIONS; do
  ...
  # Process and combine data for this region - FIXED: Using files instead of command line args
...
# Combine all region data into final JSON - FIXED: Using file-based approach
echo "Combining data from all regions..."

# Create regions array file by file (avoid command line length limits)
...
# Merge base.json with regions array using jq with file input
